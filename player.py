...
class Player:
    ...
    PROGRESS_COLOR_BACK = LIGHT_LAVENDOR
    ...
    """
	A Scrabble game typically ends when one of the following conditions is met:
    
	All tiles have been drawn and one player has used all of their remaining tiles. 
    In this case, the game ends, and the player who used all their tiles receives 
    ...
    def isGameOver(self):
        ...
        return self.theBag.isEmpty() and len(self.tray) == 0
    ...
    def executeTurn(self, isFirstTurn, DISPLAYSURF):
            ...
            for x in range(board.Board.GRID_SIZE):
                for y in range(board.Board.GRID_SIZE):
                    if self.theBoard.squares[x][y][0] != None:
                        ...
                        # DOWN
                        if (
                            y < board.Board.GRID_SIZE - 1
                            and self.theBoard.squares[x][y + 1][0] == None
                        ):
                            if not (x, y + 1) in seeds:
                                seeds.append((x, y + 1))
                        # LEFT
                        ...
                        # RIGHT
                        if (
                            x < board.Board.GRID_SIZE - 1
                            and self.theBoard.squares[x - 1][y][0] == None
                        ):
                            if not (x + 1, y) in seeds:
                                seeds.append((x + 1, y))
        ...
        for x, y in seeds:
            for lo in range(0, len(self.tray)):
                for hi in range(0, len(self.tray) - lo):
                    ...
                    # Build left
                    while xPos > 0 and (
                        loCount < lo or self.theBoard.squares[xPos][yPos][0] != None
                    ):
                        loCount += 1
                        horz.insert(
                            0, ((xPos, yPos), self.theBoard.squares[xPos][yPos][0])
                        )
                        xPos -= 1
                    # Build right
                    xPos, yPos = x + 1, y
                    while xPos < board.Board.GRID_SIZE - 1 and (
                        hiCount < hi or self.theBoard.squares[xPos][yPos][0] != None
                    ):
                        hiCount += 1
                        horz.append(
                            ((xPos, yPos), self.theBoard.squares[xPos][yPos][0])
                        )
                        xPos += 1
                    ...
                    # Build down
                    xPos, yPos = x, y + 1
                    while yPos < board.Board.GRID_SIZE - 1 and (
                        hiCount < hi or self.theBoard.squares[xPos][yPos][0] != None
                    ):
                        hiCount += 1
                        vert.append(
        ...
        tileSlotsMap = (
            {}
        )  # This hashes (x1,y1,x2,y2) to True if there is a tileslot with those bounds
        i = 0
        ...
        for tileSlot in tileSlots:
            ...
            (points, tiles, blanks) = self.tryEverything(
                isFirstTurn, wordBuilt, emptySlots, self.tray
            )
            if points > maxPoints:
        ...
        if maxTiles != None and maxTiles != []:
            ...
            if board.Board.DEBUG_ERRORS:
                Player.aiStats.updateTiming(
                    time.time() - startTime, self.maxWordTimeStamp - startTime
                )
                lettersUsed = []
        ...
        if board.Board.DEBUG_ERRORS:
            endTime = time.time()
            timeSpent = endTime - startTime + 0.00001
            ...
            totalValidationTime = (
                self.theBoard.quickValidationTime
                + self.theBoard.crosswordValidationTime
                + self.theBoard.dictionaryValidationTime
                + self.theBoard.scoringTime
            ) + 0.00001
            percentQuickValidation = (
                self.theBoard.quickValidationTime / totalValidationTime
            )
            percentCrosswordValidation = (
                self.theBoard.crosswordValidationTime / totalValidationTime
            )
            percentDictionaryValidation = (
                self.theBoard.dictionaryValidationTime / totalValidationTime
            )
            ...
            print(
                "\t\t"
                + str(percentInitializing)
                + " percent was spent initializing seed positions."
            )
            print(
                "\t\t"
                + str(percentValidating)
                + " percent was spent validating, in total."
            )
            print(
                "\t\t"
                + str(percentLookup)
                + " percent was spent on dictionary lookups."
            )
            print("\t" + str(len(seeds)) + " number of seed positions considered.")
            print(
                "\t"
                + str(100.0 * self.numValidations / (self.numRawValidations + 1) - 100)
                + " percent complexity increase due to blanks."
            )
            print("\t" + str(self.numValidations) + " validations.")
            print(
                "\t"
                + str(self.numValidations - self.theBoard.invalidWordCount)
                + " of those plays were possible."
            )
            print(
                "\t"
                + str(self.theBoard.crosswordErrors)
                + " errors from invalid crossword formation."
            )
            print(
                "\t"
                + str(1.0 * self.numValidations / (len(tileSlots) + 1))
                + " average validations per slot."
            )
            print("\t" + str(len(tileSlots)) + " slot sets considered.")
            print(
                "\t"
                + str(100.0 * numEliminated / originalSize)
                + " percent reduction by using trimming."
            )
    ...
    def tryEverything(self, isFirstTurn, word, slots, trayTiles, tilesPlaced=[]):
        ...
        if len(slots) == 0:
            if (
                board.Board.DEBUG_ERRORS
            ):  # Some quick metrics for analyzing the algorithm
            ...
            if not " " in spelling:
                if (
                    self.theBoard.dictionary.isValid(spelling, self.usageLimit)
                    or len(slots) == 1
                ):
                    if board.Board.DEBUG_ERRORS:
                    ...
                    (score, dummy, seedRatio) = self.theBoard.validateWords(
                        isFirstTurn, tilesPlayed=tilesPlaced, vocabulary=self.usageLimit
                    )
                else:
                    score = -1000
            ...
            # Otherwise, we need to try to find a letter choice that works
                ...
                blankAssignments = self.theBoard.dictionary.matchWithBlanks(
                    spelling, vocabulary=self.usageLimit
                )
                ...
                if len(blankAssignments) > 0:
                    for assignment in blankAssignments:
                        ...
                        (score, dummy, seedRatio) = self.theBoard.validateWords(
                            isFirstTurn,
                            tilesPlayed=tilesPlaced,
                            vocabulary=self.usageLimit,
                        )
            ...
            score += self.heuristic.adjust(
                trayTiles=self.tray, playTiles=tilesPlaced, seedRatio=seedRatio
            )
        ...
        # RECURSIVE CASE: Try applying all possible tiles to the slot, return the maximimum score
            ...
            for tile in trayTiles:
                ...
                (score, tilesTried, blankAssignment) = self.tryEverything(
                    isFirstTurn, word, slots[1:], trayRemaining, newTilesPlaced
                )
                if score > maxScore:
    ...
    def updateProgressBar(self, progress, DISPLAYSURF):
        progText = Player.FONT.render(
            "Thinking...", True, Player.FONT_COLOR, Player.BACKGROUND_COLOR
        )
        ...
        pygame.draw.rect(
            DISPLAYSURF,
            Player.PROGRESS_COLOR_BACK,
            (
                Player.PROGRESS_LEFT,
                Player.PROGRESS_TOP + Player.PROGRESS_MARGIN,
                Player.PROGRESS_WIDTH,
                Player.PROGRESS_HEIGHT,
            ),
        )
        width = progress * Player.PROGRESS_WIDTH
        pygame.draw.rect(
            DISPLAYSURF,
            Player.PROGRESS_COLOR_FRONT,
            (
                Player.PROGRESS_LEFT,
                Player.PROGRESS_TOP + Player.PROGRESS_MARGIN,
                width,
